...
class double_DQN:
    ...
    def create_model(self):
        ...
        model.add(Dense(10, input_dim=state_shape[0], activation="relu"))
...
def main():
    ...
    for i_episode in range(episodes):
        ...
        if i_episode >= 300:
            ave_reward_total = np.mean(reward_total_chain[range(i_episode - 20, i_episode)])
            if ave_reward_total > -150:
                break
    ...
    ddqn_agent.save_model("model_2layers_1")
    np.save("reward_total_chain_2layers_1.npy", reward_total_chain)
...
result = np.load('reward_total_chain_2layers_1.npy')

plt.figure()
plt.plot(result[range(846)])
plt.xlabel('Episodes')
plt.ylabel('Total Rewards')
plt.savefig("train_reward_2layers_1.png")
